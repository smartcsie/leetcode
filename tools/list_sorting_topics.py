#!/usr/bin/env python3
"""
list_sorting_topics.py

列出目前 metadata/*.yml 裡，'sorting' 或 'sorting-*' 開頭分類主題底下
實際收錄的題目，方便逐題核對要不要拆進更細的子分類（custom comparator、
counting sort、bucket sort、radix sort、merge sort 應用等）。

用法：
    python3 list_sorting_topics.py [metadata_dir]
...
def main():
    ...
    for fpath in sorted(glob.glob(os.path.join(meta_dir, '*.yml'))):
        ...
        for sol in data.get('solutions', []):
            for topic in to_list(sol.get('topics')):
                if not (topic == 'sorting' or topic.startswith('sorting-')):
                    continue
    ...
    if not by_topic:
        print("沒有找到任何 sorting / sorting-* 分類的題目。")
        return
