...
列出目前 metadata/*.yml 裡，所有 dp-* 開頭（以及 dynamic-programming
本身）分類主題底下實際收錄的題目，方便逐題核對分類對不對。

用法：
    python3 list_dp_topics.py [metadata_dir]
...
def main():
    ...
    for fpath in sorted(glob.glob(os.path.join(meta_dir, '*.yml'))):
        ...
        for sol in data.get('solutions', []):
            for topic in to_list(sol.get('topics')):
                if not (topic == 'dynamic-programming' or topic.startswith('dp-')):
                    continue
    ...
    if not by_topic:
        print("沒有找到任何 dynamic-programming / dp-* 分類的題目。")
        return
