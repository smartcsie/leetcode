...
def build_topic_indexes(problems, topics_out_dir):
    ...
    familiarity_order = {'生疏': 0, '注意': 1, '熟練': 2}

    for topic, rows in topic_rows.items():
        rows.sort(key=lambda r: (familiarity_order.get(r['familiarity'], 3), r['number']))
        lines = [f"# {topic}", '',
        ...
        for r in rows:
            ...
            familiarity_cell = familiarity_badges.get(r['familiarity'], '')
            lines.append(f"| {r['number']} | {title_cell} | "
                         f"{r['difficulty']} | {tags_str} | {file_cell} | {r['time']} | {r['space']} | {familiarity_cell} |")
