 ...
 * 空間複雜度：O(1) - 原地搜尋。 設計思路： 1. 若 `nums[mid] < nums[right]`：最小值必在左半區間（含 mid）。 2. 若 `nums[mid] > nums[right]`：最小值必在右半區間（不含 mid）。 3. 若 `nums[mid] == nums[right]`：無法判斷，將 `right--` 以避開一個潛在的重複值，確保最小值仍在區間內。
 */
