...
[在 LeetCode 上查看](https://leetcode.com/problems/populating-next-right-pointers-in-each-node-ii/)

**難度:** 🟡 Medium　**標籤:** Tree Bfs Level Order, BFS, Queue　**時間:** O(N)　**空間:** O(N)　**熟悉度:** 🟡 練習過　🏆 **最佳解**
...
BFS加上node->next = (i == 0) ? nullptr : q.front();
i == 0是判斷是否為最右邊的node
