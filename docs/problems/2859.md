# 2859. Sum of Values at Indices With K Set Bits

[在 LeetCode 上查看](https://leetcode.com/problems/sum-of-values-at-indices-with-k-set-bits/)

**難度:** 🟢 Easy　**標籤:** Bit Manipulation, Built-in Function　**時間:** O(N)　**空間:** O(1)　**熟悉度:** 🟡 練習過

**技巧:** __builtin_popcount

```cpp
/**
 * 題目：2859. Sum of Values at Indices With K Set Bits
 * 難度：簡單 (Easy)
 * 描述：給定一個下標從 0 開始的整數陣列 nums 和一個整數 k。
 * 請返回 nums 中滿足其索引的二進位表示中恰好有 k 個設定位（即 1 的個數）的元素總和。
 *
 * 時間複雜度：O(N)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 1. 遍歷陣列的每個索引 i（從 0 到 nums.size() - 1）。
 * 2. 使用 GCC 內建函數 __builtin_popcount(i) 計算索引 i 的二進位中有多少個 1。
 * 3. 若計數等於 k，則將該索引對應的 nums[i] 累加到總和中。
 */

class Solution {
public:
    int sumIndicesWithKSetBits(vector<int>& nums, int k) {
        int sum = 0;
        for(int i = 0; i < nums.size(); i++) {
            if(__builtin_popcount(i) == k) sum += nums[i];
        }
        return sum;
    }
};
```
