...
[在 LeetCode 上查看](https://leetcode.com/problems/k-diff-pairs-in-an-array/)

**難度:** 🟡 Medium　**標籤:** Hash Table Existence　**時間:** O(NlogN) - 排序後雙指針或二分搜尋。　**空間:** O(N) - 排序空間 O(logN)，set O(N)。　**熟悉度:** 🟠 再練習
...
> 兩種做法<br>
> 或是存數對的前面<br>
> if(seen.count(x - k))  ans.insert(x - k);<br>
> if(seen.count(x + k))  ans.insert(x);<br>
> 或是存數對的後面<br>
> if(seen.count(x - k))  ans.insert(x);<br>
> if(seen.count(x + k))  ans.insert(x + k);<br>
