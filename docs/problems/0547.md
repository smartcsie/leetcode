...
[在 LeetCode 上查看](https://leetcode.com/problems/number-of-provinces/)

**難度:** 🟡 Medium　**標籤:** Union Find　**時間:** O(N²·α(N)) - 需雙重迴圈遍歷 n x n 鄰接矩陣（上三角，含對角線），　**空間:** O(N) - id 與 rank 兩個陣列各佔用 O(N) 空間。　**熟悉度:** 🔴 生疏
