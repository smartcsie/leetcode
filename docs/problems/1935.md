# 1935. Maximum Number of Words You Can Type

[在 LeetCode 上查看](https://leetcode.com/problems/maximum-number-of-words-you-can-type/)

**難度:** 🟢 Easy　**標籤:** String Character Classification, Stream　**時間:** O(N*M)　**空間:** O(1)　**熟悉度:** 🟢 熟練

**📅 練習歷程:**

- ✅ **2026-09-25** 對

```cpp
/**
 * 題目：1935. Maximum Number of Words You Can Type
 * 難度：簡單 (Easy)
 * 描述：計算在給定壞掉鍵盤的情況下，有多少個完整的單字可以被完全輸入。
 *
 * 時間複雜度：O(N*M)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 1. 使用 istringstream 將 text 切分成多個單字。
 * 2. 針對每個單字進行檢查，若單字中包含任何一個 brokenLetters，則該單字無法輸入。
 * 3. 統計未被標記為「損壞」的單字數量。
 */

class Solution {
public:
    int canBeTypedWords(std::string text, std::string brokenLetters) {
        int mask = 0;
        for(const char& c : brokenLetters) mask |= 1 << (c - 'a');
        int ans = 0;
        string word;
        istringstream iss(text);
        while(iss >> word){
            int submask = 0;
            for(const char& c : word) submask |= 1 << (c - 'a');
            if(!(mask & submask)) ans++;
        }
        return ans;
    }
};
```
