# 2220. Minimum Bit Flips to Convert Number

[在 LeetCode 上查看](https://leetcode.com/problems/minimum-bit-flips-to-convert-number)

**難度:** 🟢 Easy　**標籤:** Bit Manipulation　**時間:** O(log(max(START,GOAL)))　**空間:** O(1)　**熟悉度:** 🟢 熟練

**📅 練習歷程:**

- ✅ **2026-09-18** 對

**技巧:** __builtin_popcount

```cpp
/**
 * 題目：2220. Minimum Bit Flips to Convert Number (轉換數字的最少位元翻轉次數)
 * 難度：簡單 (Easy)
 * 描述：計算將 `start` 轉換為 `goal` 所需的最少位元翻轉次數。
 *
 * 時間複雜度：O(logN)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * (XOR + Hamming Weight)：
 * 1. XOR (^) 運算：若兩個位元相同則為 0，不同則為 1。
 * 因此，`start ^ goal` 的結果中，所有為 1 的位元即為需要翻轉的地方。
 * 2. 統計 1 的個數：
 * - 方法一：你使用的 `while` 迴圈搭配位移運算 (>> 1)。
 * - 方法二 (進階)：使用內建函數 `__builtin_popcount(xorVal)`，執行效率極高。
 */

class Solution {
public:
    int minBitFlips(int start, int goal) {
        return __builtin_popcount(start ^ goal);
    }
};
```

> start和goal的Hamming Distance<br>
<br><br>

**相關題目:** [461](../problems/0461.md)
