...
[在 LeetCode 上查看](https://leetcode.com/problems/merge-two-2d-arrays-by-summing-values/)

**難度:** 🟢 Easy　**標籤:** Two Pointers Same　**時間:** O(N+M) - 由於兩個輸入陣列皆已排序，利用雙指標（Two Pointers）線性掃描一次即可完成，耗時與兩陣列長度總和成正比。　**空間:** O(1) - 僅使用常數個指標與變數（i, j, m, n），並透過 `ans.reserve(m + n)` 預先分配記憶體，避免動態擴容開銷。　**熟悉度:** 🟢 熟練
