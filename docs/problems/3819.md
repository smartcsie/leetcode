# 3819. Rotate Non Negative Elements

[在 LeetCode 上查看](https://leetcode.com/problems/rotate-non-negative-elements/)

**難度:** 🟢 Easy　**標籤:** Array, Two Pointers　**時間:** O(N)　**空間:** O(N)　**熟悉度:** 🟡 練習過

**技巧:** rotate

```cpp
/**
 * 題目：3819. Rotate Non Negative Elements
 * 難度：簡單 (Easy)
 * 描述：將陣列中所有非負數向左旋轉 k 位，負數保持原位。
 *
 * 時間複雜度：O(N)
 * 空間複雜度：O(N)
 *
 * 解法思路：
 * 1. 使用 reserve() 減少動態記憶體分配次數。
 * 2. 透過 range-based for loop 與 reference 寫回原陣列，提升可讀性。
 */

#include <vector>
#include <algorithm>

class Solution {
public:
    vector<int> rotateElements(vector<int>& nums, int k) {
        // 1. 預先收集非負數，並使用 reserve 減少開銷
        vector<int> nonNegs;
        nonNegs.reserve(nums.size()); 
        for(const int num : nums) {
            if(num >= 0) nonNegs.push_back(num);
        }
        
        // 若無非負數或無需旋轉，直接返回
        if (nonNegs.empty()) return nums;
        
        // 2. 規範化 k 的值，確保 rotate 不會越界
        int n = static_cast<int>(nonNegs.size());
        k %= n;
        
        // 3. 使用標準庫原地旋轉，效率極高 (三段翻轉法)
        rotate(nonNegs.begin(), nonNegs.begin() + k, nonNegs.end());
        
        // 4. 將旋轉後的結果放回原陣列
        int j = 0;
        for(int& num : nums) {
            if(num >= 0) {
                num = nonNegs[j++];
            }
        }
        
        return nums;
    }
};
```

> **注意devide by zero，if (nonNegs.empty()) return nums;**<br>
> **left rotate，rotate(nums.begin(), nums.begin() + k, nums.end());**<br>
> **right rotate，rotate(nums.rbegin(), nums.rbegin() + k, nums.rend());**<br>
