# 48. Rotate Image

[在 LeetCode 上查看](https://leetcode.com/problems/rotate-image/)

**難度:** 🟡 Medium　**標籤:** 2d Array　**時間:** O(N²)　**空間:** O(1)　**熟悉度:** 🟢 熟練　👑 **代表題（Matrix Rotate）**

**📅 練習歷程:**

- ✅ **2026-09-16** 對
- ✅ **2026-09-16** 對
- ✅ **2026-09-16** 對

**技巧:** matrix rotate、matrix transpose

```cpp
/**
 * 題目：48. Rotate Image (旋轉圖像)
 * 難度：中等 (Medium)
 * 描述：給定一個 n x n 的二維矩陣 matrix 表示一個圖像。
 * 請將圖像順時針旋轉 90 度。你必須在「原地」修改，不要使用另一個矩陣。
 *
 * 時間複雜度：O(N²)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 順時針旋轉 90 度可以拆解為兩步操作：
 * 1. 轉置矩陣 (Transpose)：
 * 將矩陣沿著主對角線 (左上到右下) 翻轉，即交換 matrix[i][j] 與 matrix[j][i]。
 * 此時：原本的行變成了列。
 * 2. 左右翻轉 (Reverse Each Row)：
 * 將矩陣的每一行進行左右對調。
 * 此時：原本在左側的元素移到了右側，完成了順時針 90 度的變換。
 */

class Solution {
public:
    void rotate(vector<vector<int>>& matrix) {
        int n = matrix.size();
        // 1. 轉置矩陣 (Transpose)
        // 注意：j 從 i 開始，只處理對角線的一側，否則換過去又換回來等於沒換
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                swap(matrix[i][j], matrix[j][i]);
            }
        }
        // 2. 每一行進行左右翻轉 (Reverse each row)
        for (int i = 0; i < n; ++i) {
            // 使用 std::reverse 直接翻轉整行
            reverse(matrix[i].begin(), matrix[i].end());
        }
    }
};
```

> **先求matrix transpose，每個row再取reverse**<br>

**相關題目:** [867](../problems/0867.md), [1886](../problems/1886.md)
