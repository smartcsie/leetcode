# 383. Ransom Note

[在 LeetCode 上查看](https://leetcode.com/problems/ransom-note)

**難度:** 🟢 Easy　**標籤:** Hash Table Intersection, Hash Table　**時間:** O(M+N)　**空間:** O(1)　**熟悉度:** 🟠 再練習　👑 **代表題（compare count one side limit）**

**📅 練習歷程:**

- ✅ **2026-09-19** 對

```cpp
/**
 * 題目：383. Ransom Note (贖金信)
 * 難度：簡單 (Easy)
 * 描述：判斷 ransomNote 是否能由 magazine 中的字母組成。
 *
 * 時間複雜度：O(M+N)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 使用 `int count[26]` 取代 `unordered_map`，減少雜湊計算開銷，
 * 對於小範圍字元集（26個字母），陣列存取速度快於 Map。
 */

class Solution {
public:
    bool canConstruct(string ransomNote, string magazine) {
        vector<int> count(26, 0);
        for(const char& c: magazine) count[c - 'a']++;
        for(const char& c: ransomNote) {
            if(!count[c - 'a']--) return false;
        }
        return true;
    }
};
```
