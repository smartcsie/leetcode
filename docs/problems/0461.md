# 461. Hamming Distance

[在 LeetCode 上查看](https://leetcode.com/problems/hamming-distance)

## 解法：hamming-distance

**難度:** 🟢 Easy　**標籤:** Bit Manipulation　**時間:** O(K)　**空間:** O(1)　**熟悉度:** 🟡 練習過　👑 **代表題（__builtin_popcount）**

**📅 練習歷程:**

- ✅ **2026-09-18** 對

```cpp
/**
 * 題目：461. Hamming Distance (漢明距離)
 * 難度：簡單 (Easy)
 * 描述：計算兩個整數之間的漢明距離（即二進位表示中位元不同的位置數量）。
 *
 * 時間複雜度：O(K)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 1. XOR 運算：x ^ y 的結果中，每一位為 1 代表該位置的 bits 不同。
 * 2. 統計 1 的個數：使用 Brian Kernighan's Algorithm，每次執行 xorVal &= (xorVal - 1)
 * 都能精準地消除最右側的一個 1，直到 xorVal 變成 0 為止。
 */

class Solution {
public:
    int hammingDistance(int x, int y) {
        return popcount(static_cast<unsigned int>(x ^ y));
    }
};
```

> x和y做互斥或成為z，取z的2進制1的個數<br>

**相關題目:** [2220](../problems/2220.md)

---

## 解法：bit-manipulation-xor

**難度:** 🟢 Easy　**標籤:** Bit Manipulation Xor　**時間:** O(N)　**空間:** O(1)　**熟悉度:** 🟢 熟練　👑 **代表題（Hamming Distance）**

**📅 練習歷程:**

- ✅ **2026-09-18** 對

```cpp
/**
 * 題目：461. Hamming Distance
 * 難度：簡單 (Easy)
 * 描述：計算兩個整數對應二進位位元不同的個數。
 *
 * 時間複雜度：O(1)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * （Brian Kernighan's Algorithm）：
 * XOR 找出不同位元，再用 n &= (n-1) 逐一消除最低位的 1 並計數。
 */
class Solution {
public:
    int hammingDistance(int x, int y) {
        int xorVal = x ^ y;
        int count = 0;
        while (xorVal) {
            xorVal &= (xorVal - 1);  // 每次消除最低位的 1
            count++;
        }
        return count;
    }
};
```

---
