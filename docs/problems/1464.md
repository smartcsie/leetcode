# 1464. Maximum Product of Two Elements in an Array

[在 LeetCode 上查看](https://leetcode.com/problems/maximum-product-of-two-elements-in-an-array/)

**難度:** 🟢 Easy　**標籤:** Array Min Max, Greedy, Simulation　**時間:** O(N)　**空間:** O(1)　**熟悉度:** 🟢 熟練

**📅 練習歷程:**

- ✅ **2026-09-15** 對

**技巧:** Array Min Max With Repeat

```cpp
/**
 * 題目：1464. Maximum Product of Two Elements in an Array
 * 難度：簡單 (Easy)
 * 描述：給你一個整數陣列 nums，請你選擇兩個不同的索引 i 和 j，
 * 使 (nums[i]-1)*(nums[j]-1) 的值最大。請返回最大值。
 *
 * 時間複雜度：O(N)
 * 空間複雜度：O(1)
 *
 * 解法思路：
 * 1. 要讓乘積最大，我們需要選出陣列中數值最大的兩個數字。
 * 2. 利用 `std::pair<int, int>` 紀錄最大值（first）與第二大值（second）。
 * 3. 走訪整個 nums 陣列：
 * - 如果當前元素大於最大值，則將原最大值降級為第二大值，並更新最大值。
 * - 否則，如果當前元素大於第二大值，則更新第二大值。
 * 4. 迴圈結束後，計算 `(maxVal.first - 1) * (maxVal.second - 1)` 並回傳。
 */

class Solution {
public:
    int maxProduct(std::vector<int>& nums) {
        int mx1 = 0, mx2 = 0;
        for(const int& x : nums) {
            if(x > mx1) {
                mx2 = mx1;
                mx1 = x;
            } else if(x > mx2) {
                mx2 = x;
            }
        }
        return (mx1 - 1) * (mx2 - 1);
    }
};
```

[比大小]
