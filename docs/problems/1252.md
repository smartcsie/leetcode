# 1252. Cells with Odd Values...

[在 LeetCode 上查看](https://leetcode.com/problems/cells-with-odd-values-in-a-matrix/)

**難度:** 🟢 Easy　**標籤:** 2d Array, Math　**時間:** O(K+MN)　**空間:** O(M+N)　**熟悉度:** 🟢 熟練

**📅 練習歷程:**

- ✅ **2026-09-16** 對

```cpp
/**
 * 題目：1252. Cells with Odd Values in a Matrix
 * 難度：簡單 (Easy)
 * 描述：在 m x n 矩陣中，根據給定的 indices 對行列進行增加，最後統計矩陣中值為奇數的單元格數量。
 *
 * 時間複雜度：O(K+M*N)
 * 空間複雜度：O(M+N)
 *
 * 解法思路：
 * 1. 矩陣中位置 (i, j) 的最終值 = row[i] + col[j]。
 * 2. 位置 (i, j) 為奇數的條件是：row[i] 與 col[j] 一個為奇數，另一個為偶數。
 * 3. 使用 XOR 運算：`(row[i] % 2) ^ (col[j] % 2)` 即為當前格是否為奇數的布林值。
 */

class Solution {
public:
    int oddCells(int m, int n, std::vector<std::vector<int>>& indices) {
        vector<int> rows(m, 0);
        vector<int> cols(n, 0);
        for(const vector<int> idx : indices) {
            rows[idx[0]]++;
            cols[idx[1]]++;
        }
        int count = 0;
        for(int i = 0; i < m; i++) {
            for(int j = 0; j < n; j++) {
                if((rows[i] & 1) ^ (cols[j] & 1)) count++;
            }
        }
        return count;
    }
};
```
