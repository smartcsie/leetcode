# 1647. Minimum Deletions...

[在 LeetCode 上查看](https://leetcode.com/problems/minimum-deletions-to-make-character-frequencies-unique/)

**難度:** 🟡 Medium　**標籤:** Greedy, Hash Set　**時間:** O(N+KlogK)　**空間:** O(K)　**熟悉度:** 🟠 再練習

```cpp
/**
 * 題目：1647. Minimum Deletions to Make Character Frequencies Unique
 * 難度：中等 (Medium)
 * 描述：調整字元頻率使其皆唯一，計算最少的刪除字元數。
 *
 * 時間複雜度：O(N+KlogK) - N 為字串長度，K 為字母總數 (26)。
 * 空間複雜度：O(K)。
 *
 * 解法思路：
 * 1. 使用陣列統計每個字元的頻率。
 * 2. 將頻率進行排序，以便由大到小檢查重複性。
 * 3. 利用 Hash Set 記錄已出現的頻率，若發生衝突，則遞減當前頻率直到唯一或歸零。
 */

class Solution {
public:
    int minDeletions(string s) {
        vector<int> counts(26, 0);
        for(const char& c : s) counts[c - 'a']++;
        unordered_set<int> seen;
        int ans = 0;
        for(int i = 0; i < 26; i++) {
            while(counts[i] && !seen.insert(counts[i]).second) {
                counts[i]--;
                ans++;
            }
        }
        return ans;
    }
};
```
