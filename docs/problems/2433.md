# 2433. Find The Original Array of Prefix Xor

[在 LeetCode 上查看](https://leetcode.com/problems/find-the-original-array-of-prefix-xor/)

**難度:** 🟡 Medium　**標籤:** Bit Manipulation Xor, Prefix XOR Inverse　**時間:** O(N)　**空間:** O(N)　**熟悉度:** 🟢 熟練

**📅 練習歷程:**

- ✅ **2026-09-18** 對

```cpp
/**
 * 題目：2433. Find The Original Array of Prefix Xor (找出前綴異或的原陣列)
 * 難度：中等 (Medium)
 * 描述：給你一個長度為 n 的整數陣列 pref。找出並返回滿足以下條件的原陣列 arr：
 * pref[i] = arr[0] ^ arr[1] ^ ... ^ arr[i]
 *
 * 時間複雜度：O(N)
 * 空間複雜度：O(N)
 *
 * 解法思路：
 * 1. XOR 運算的自我反轉性質：
 * - 在位元運算中，XOR 具備一個非常重要的特性：若 A ^ B = C，則 A ^ C = B。
 * 2. 推導還原公式：
 * - 根據題目定義，pref[i-1] = arr[0] ^ ... ^ arr[i-1]，而 pref[i] = arr[0] ^ ... ^ arr[i-1] ^ arr[i]。
 * - 將兩式進行 XOR 運算可得：pref[i - 1] ^ pref[i] = arr[i]。
 * 3. 實作步驟：
 * - 第一個元素直接對應 pref[0]（即 ans[0] = pref[0]）。
 * - 從索引 1 開始，透過 ans[i] = pref[i - 1] ^ pref[i] 依序還原後續元素。
 */

class Solution {
public:
    std::vector<int> findArray(std::vector<int>& pref) {
        vector<int> prefix(pref.size());
        prefix[0] = pref[0];
        for(int i = 1; i < n; i++) {
            prefix[i] =  pref[i] ^ pref[i - 1];
        }
        return prefix;
    }
};
```

> **所求prefix[n] = prefix[0]^prefix[1]^.....prefix[n-1]**<br>
> **等價prefix[n] = pref[n]^pref[n-1]**<br>

**相關題目:** [1720](../problems/1720.md)
